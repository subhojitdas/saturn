...
class LogisticRegressionHeartDiseaseRegularization:
    ...
    def fit(self, X_train, y_train, lambda_regularization=0.1):
        ...
        for i in range(10000):
            ...
            gradient = (1/feature_sample) * (x_with_bias.T @ (h - y_train)) + (lambda_regularization/feature_sample) * theta
            gradient[0] -= (lambda_regularization / feature_sample) * theta[0]  # No regularization for bias term
            theta -= gradient
...
model = LogisticRegressionHeartDiseaseRegularization()
